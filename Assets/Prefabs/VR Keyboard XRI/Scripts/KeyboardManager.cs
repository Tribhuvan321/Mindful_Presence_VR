...
namespace Keyboard
{
    public class KeyboardManager : MonoBehaviour
    {
        ...
        private void CheckTextLength()
        {
            ...
            // Remove the condition that limits the text length
            bool keysEnabled = true; // Always enable keys
            keyChannel.RaiseKeysStateChangeEvent(keysEnabled);

            // Enable the enter button if there are characters in the output field
            enterButton.interactable = currentLength >= minCharacters;

            // Always enable the delete button, regardless of the text length
            deleteButton.interactable = true;

            // No longer disable shift/caps lock based on maximum text length
            DeactivateShift();
            capsLockActive = false;
            UpdateShiftButtonAppearance();
        }
